...
$(function () {
    ...
    function updateUserWallet() {
        // Find and update the user in userArr
    ...
    function drawChart(selectedIndex) {
        ...
        //     minMin=Math.min(market[i].coins[selectedIndex].min)      
        // }
        ...
        if (count < MAX_BARS) {
            ...
            $(".stick:first").remove();
            $(".bar:first").remove();
            $(".dotted-line:first").remove(); // Remove old lines when max bars are reached
        ...
        $("#chart-container").append(`
            <div class='dotted-line' style='
                ...
                left: 0;
                z-index: 1;
            '><p style='text-align:right;color:#eee'>$${close}</p></div>
        `);
    ...
    function animateIcon(){
        selectedCrypto = $(".crypto-icons img").attr('alt').toUpperCase();
